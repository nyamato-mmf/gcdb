...
let Colors_array_metropolitans_population_timeseries = [];
let Color_obj_metropolitans_population_timeseries = {};

// Wait for DOM to be fully loaded
document.addEventListener("DOMContentLoaded", function() {
    // Set up canvas size management
    const canvas = document.getElementById("metropolitan-population-timeseries");
    ...
    fetch("./data/color_code.csv")
        ...
        .then(csvText => {
            ...
            Colors_array_metropolitans_population_timeseries = Papa.parse(csvText, { header: true, dynamicTyping: true }).data;
            
            // Convert array into an object for easier lookup
            Color_obj_metropolitans_population_timeseries = Colors_array_metropolitans_population_timeseries.reduce((acc, entry) => {
                acc[entry.Country_Code] = entry.Color_Code;
                return acc;
            }, {});
            
            // Now load population data
            return fetch("./data/csv/population/metropolitans/metropolitans-population-timeseries.csv");
        })
        .then(response => response.text())
        .then(csvText => {
            ...
            // Now, cityParam is guaranteed to be a key in cityMap (e.g., "TYO", "LON", etc.).
            // The `|| cityMap["TYO"]` part is no longer necessary as the default is handled above.
            const countryCode = cityMap[cityParam]; 
            
            // Parse CSV into an array of objects
            const data = Papa.parse(csvText, { header: true, dynamicTyping: true }).data;
            
            // Remove empty or invalid rows
            const filteredData = data.filter(entry => entry["Country_Code"] === countryCode);

            // Extract unique years dynamically (excluding "Country_Name" and "Country_Code")
            const excludeCols = ["Country_Name", "Country_Code"];
            ...
            const datasets = filteredData.map(entry => ({
                label: entry["Country_Name"], // Country name for legend
                data: years.map(year => Math.floor((entry[year] ?? 0) / 1000000)), // Prevent NaN by using `?? 0`
                borderColor: Color_obj_metropolitans_population_timeseries[entry["Country_Code"]] || "gray", // Predefined colour or gray
                fill: false
            }));
            
            // Create the chart
            new Chart("metropolitan-population-timeseries", {
                type: "line",
                data: {
