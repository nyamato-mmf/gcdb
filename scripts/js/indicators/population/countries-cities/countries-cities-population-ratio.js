...
let Colors_array_countries_cities_population_ratio = [];
let Color_obj_countries_cities_population_ratio = {};

// Wait for DOM to be fully loaded
document.addEventListener("DOMContentLoaded", function() {
    // Set up canvas size management
    const canvas = document.getElementById("countries-cities-population-ratio");
    ...
    fetch("./data/color_code.csv")
        ...
        .then(csvText => {
            ...
            Colors_array_countries_cities_population_ratio = Papa.parse(csvText, { header: true, dynamicTyping: true }).data;

            // Convert array into an object for easier lookup
            Color_obj_countries_cities_population_ratio = Colors_array_countries_cities_population_ratio.reduce((acc, entry) => {
                acc[entry.Country_Code] = entry.Color_Code;
                return acc;
            }, {});
            
            // Now load population data
            return fetch("./data/csv/population/countries-cities/countries-cities-population-ratio.csv");
        })
        .then(response => response.text())
        .then(csvText => {
            // Parse CSV into an array of objects
            ...
            // Create the chart
            new Chart("countries-cities-population-ratio", {
                type: "bar",
                data: {
